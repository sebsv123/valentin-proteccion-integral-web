...
const product = products.find((item) => item.slug === 'mascotas')!;
const english = getLocalizedProduct(product, 'en');
const url = 'https://valentinproteccionintegral.com/en/insurance/pet-insurance';
const esUrl = 'https://valentinproteccionintegral.com/seguros/mascotas';
...
export default async function EnglishPets({ params }: { params: Promise<{ locale: string }> }) { const { locale } = await params; if (locale !== 'en') notFound(); return <ProductPageView slug="mascotas" locale="en" />; }
