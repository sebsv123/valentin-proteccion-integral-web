  ...
  return (
    <>
      ...
      <main>
        ...
        <SubpageHero subpage={subpage} showActions={!isComprehensive} />
        {isComprehensive ? <HealthComprehensiveGuidance locale={locale} /> : <ProductDecisionGrid product={product} locale={locale} />}
        <ProductFaqSection product={faqProduct} locale={locale} contextualLinks={!isComprehensive} />
        ...
        <ProductCTASection product={product} locale={locale} suppressPriceGuarantee={isComprehensive} suppressTrustMetrics={isComprehensive} title={isComprehensive ? (locale === 'en' ? 'I want to review whether I need hospital cover' : 'Quiero revisar si necesito hospitalización') : (locale === 'en' ? `Would you like us to review ${product.label} in more detail?` : `¿Quieres que revisemos ${product.label} con más calma?`)} text={isComprehensive ? (locale === 'en' ? 'We can help you clarify how you expect to use private healthcare and what to verify in the hospital-cover policy before deciding.' : 'Te ayudamos a aclarar cómo esperas usar la sanidad privada y qué comprobar en la póliza con hospitalización antes de decidir.') : (locale === 'en' ? 'If this profile or plan fits you, we can clarify the details, answer your questions and help you decide with greater confidence.' : 'Si este perfil o modalidad encaja contigo, te ayudamos a aterrizar matices, resolver dudas y decidir con más tranquilidad.')} message={subpage.whatsappMessage} />
        {!isComprehensive && <RelatedProducts product={product} locale={locale} />}
      </main>
      <Footer />
      <StickyWhatsApp />
    </>
