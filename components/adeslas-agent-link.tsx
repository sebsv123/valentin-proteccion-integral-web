...
const DEFAULT_DISCLAIMER_ES = "Al continuar, accederás a una página externa de SegurCaixa Adeslas. Los datos que introduzcas allí se tratarán conforme a la política de privacidad de dicha entidad.";
const DEFAULT_DISCLAIMER_EN = "You will continue to an external SegurCaixa Adeslas page. Any data you enter there will be handled according to that organisation’s privacy policy.";
...
export function AdeslasAgentExclusiveBadge({ compact = false, showLogo = true, className = "" }: AdeslasAgentExclusiveBadgeProps) {
  ...
  return (
    <div className={`inline-flex max-w-full flex-wrap items-center gap-x-3 gap-y-2 rounded-xl border border-[#C8A45D]/35 bg-white px-3.5 text-[#0B2745] shadow-[0_7px_20px_rgba(0,34,68,0.09)] ${compact ? "min-h-10 py-2" : "min-h-12 py-2.5 sm:px-4"} ${className}`}>
      <span className={`font-bold tracking-[0.04em] ${compact ? "text-xs" : "text-sm"}`}>{isEnglish ? 'Exclusive agent' : 'Agente Exclusivo'}</span>
...
export function AdeslasAgentLink({ location, variant = "card", title = "También puedes solicitar contacto desde la página de Adeslas", description = "Si lo prefieres, utiliza el buscador oficial de agentes de SegurCaixa Adeslas en Boadilla del Monte para dejar tus datos y solicitar contacto con Rosa Valentín. WhatsApp y el formulario de VPI siguen disponibles para una atención directa.", linkLabel = "Solicitar contacto desde Adeslas", secondaryHref, secondaryLabel, showDisclaimer = variant !== "button", locale = 'es' }: AdeslasAgentLinkProps) {
  ...
  const link = (
    <a
      ...
      aria-label={isEnglish ? 'Open the official SegurCaixa Adeslas agent directory to contact Rosa Valentín' : 'Abrir buscador oficial de agentes de SegurCaixa Adeslas para contactar con Rosa Valentín'}
    >
      {linkLabel}
  ...
  if (variant === "button") return showDisclaimer ? <div>{link}<p className="mt-3 max-w-xl text-xs leading-5 text-[var(--muted)]">{disclaimer}</p></div> : link;

  if (variant === "preview") {
    const steps = [
      isEnglish ? ["Official page", "It opens in a new SegurCaixa Adeslas tab."] : ["Página oficial", "Se abre en una nueva pestaña de SegurCaixa Adeslas."],
      isEnglish ? ["Rosa's listing", "You will see Rosa Valentín's channel for Boadilla del Monte."] : ["Resultado de Rosa", "Verás el canal asociado a Rosa Valentín para Boadilla del Monte."],
      isEnglish ? ["Contact request", "You can leave your details in that external form."] : ["Solicitud de contacto", "Puedes dejar tus datos directamente en ese formulario externo."],
    ];

    return (
      <aside className="relative overflow-hidden rounded-[30px] border border-[#C8A45D]/35 bg-[#071D36] p-5 text-white shadow-[0_24px_70px_rgba(0,34,68,0.2)] sm:p-7 lg:p-9" aria-label={isEnglish ? 'Bridge to the official SegurCaixa Adeslas channel' : 'Puente hacia el canal oficial de SegurCaixa Adeslas'}>
        ...
        <div className="relative grid gap-7 lg:grid-cols-[1.04fr_0.96fr] lg:items-center">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#E2C078]">{isEnglish ? 'Additional official channel' : 'Canal oficial adicional'}</p>
            ...
            <p className="mt-3 text-xs leading-5 text-white/55">{isEnglish ? 'External page. No iframe. Your data will be entered on the SegurCaixa Adeslas website.' : 'Página externa. Sin iframe. Tus datos se introducirán en la web de SegurCaixa Adeslas.'}</p>
          </div>

          <div role="img" aria-label={isEnglish ? 'Informational preview of the official agent directory, showing Rosa Isabel Valentín González in Boadilla del Monte' : 'Vista previa informativa del buscador oficial de agentes, con Rosa Isabel Valentín González en Boadilla del Monte'} className="rounded-[24px] border border-white/15 bg-[#F5F7FA] p-3 text-[#0B2745] shadow-2xl sm:p-4">
            ...
            <div className="relative overflow-hidden rounded-b-[17px] border-t border-slate-100 bg-[linear-gradient(150deg,#ffffff,#eef3f8)] p-4 sm:p-6">
              <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#55718E]">{isEnglish ? 'Destination preview' : 'Vista previa del destino'}</p>
              <AdeslasAgentExclusiveBadge className="mt-3" />
              <div className="mt-4 rounded-2xl border border-[#C8A45D]/35 bg-white p-5 shadow-sm">
                <span className="inline-flex rounded-full bg-[#0B2745]/7 px-3 py-1 text-[10px] font-bold uppercase tracking-wide text-[#0B2745]">{isEnglish ? 'Agent result' : 'Resultado de agente'}</span>
                <p className="mt-4 text-lg font-extrabold leading-tight sm:text-xl">ROSA ISABEL VALENTIN GONZALEZ</p>
                <p className="mt-2 text-sm font-semibold text-[#496985]">{isEnglish ? 'Exclusive Insurance Agent' : 'Agente Exclusivo de Seguros'}</p>
                <p className="mt-1 text-sm text-slate-500">Boadilla del Monte · 28660 Madrid</p>
                <span aria-hidden="true" className="mt-5 inline-flex items-center gap-2 rounded-lg bg-[#0B2745] px-4 py-2.5 text-xs font-bold text-white">{isEnglish ? 'Information and contact' : 'Información y contacto'} <ExternalLink className="h-3.5 w-3.5" /></span>
              </div>
              <div className="mt-4 flex items-center gap-2 text-xs leading-5 text-slate-500"><ShieldCheck className="h-4 w-4 shrink-0 text-[#6B8EA8]" aria-hidden="true" />{isEnglish ? 'The live CTA opens the official page in a new tab.' : 'Al pulsar el CTA real, se abrirá la página oficial en una nueva pestaña.'}</div>
            </div>
          </div>
        </div>
        <p className="relative mt-6 border-t border-white/10 pt-4 text-xs leading-5 text-white/55">{disclaimer}</p>
      </aside>
    );
  }
  ...
  if (variant === "mini-preview") {
    return (
      <aside className="overflow-hidden rounded-[24px] border border-[#C8A45D]/35 bg-white shadow-[0_16px_45px_rgba(0,34,68,0.10)]" aria-label={isEnglish ? 'Official SegurCaixa Adeslas channel' : 'Canal oficial de SegurCaixa Adeslas'}>
        <div className="grid md:grid-cols-[minmax(0,1.15fr)_minmax(240px,0.85fr)] md:items-stretch">
          <div className="p-5 sm:p-6">
            <div className="flex items-center gap-3">
              ...
              <p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--blue-deep)]">{isEnglish ? 'Additional official channel' : 'Canal oficial adicional'}</p>
            </div>
          ...
          <div role="img" aria-label={isEnglish ? 'Informational preview of Rosa Valentín’s result in the official agent directory for Boadilla del Monte' : 'Vista previa informativa del resultado de Rosa Valentín en el buscador oficial de agentes para Boadilla del Monte'} className="border-t border-slate-200 bg-[#071D36] p-4 text-white md:border-l md:border-t-0">
            <div className="h-full rounded-2xl border border-white/15 bg-white/[0.07] p-3.5">
              ...
              <p className="mt-4 text-[10px] font-bold uppercase tracking-[0.15em] text-[#E2C078]">{isEnglish ? 'Destination preview' : 'Vista previa del destino'}</p>
              ...
              <div className="mt-2 rounded-xl bg-white p-3.5 text-[#0B2745] shadow-sm">
                ...
                <p className="mt-1 text-xs font-semibold text-[#496985]">{isEnglish ? 'Exclusive Insurance Agent' : 'Agente Exclusivo de Seguros'}</p>
                <p className="mt-1 text-xs text-slate-500">Boadilla del Monte · 28660 Madrid</p>
              </div>
              <p className="mt-3 text-[11px] leading-4 text-white/65">{isEnglish ? 'The live link opens the official page in a new tab.' : 'El enlace real abre la página oficial en una pestaña nueva.'}</p>
            </div>
          </div>
        </div>
        {showDisclaimer ? <p className="border-t border-slate-200 px-5 py-3 text-[11px] leading-5 text-[var(--muted)] sm:px-6">{disclaimer}</p> : null}
      </aside>
    );
  }
  ...
  return (
    <aside
      ...
      aria-label={isEnglish ? 'Official SegurCaixa Adeslas channel' : 'Canal oficial de SegurCaixa Adeslas'}
    >
      <div className="flex items-start gap-4">
        ...
        <div>
          <p className="text-sm font-bold uppercase tracking-[0.16em] text-[var(--blue-deep)]">
            {isEnglish ? 'Additional official channel' : 'Canal oficial adicional'}
          </p>
          ...
          {showDisclaimer ? <p className="mt-4 max-w-2xl text-xs leading-5 text-[var(--muted)]">{disclaimer}</p> : null}
        </div>
      </div>
    </aside>
  );
