      ...
      <div className="container-shell">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2">
          <Link href={locale === 'en' ? '/en/business/health-insurance' : '/empresas/salud'} className="group flex items-center justify-between gap-5 rounded-2xl border border-[rgba(15,94,156,0.16)] bg-white px-5 py-4 transition hover:-translate-y-0.5 hover:border-[var(--blue)]/40 hover:shadow-md focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[var(--blue)]/20">
            <span className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-full bg-[#e8f7f4] text-[var(--blue)]"><Building2 className="h-5 w-5" aria-hidden="true" /></span><span><span className="block text-[11px] font-bold tracking-[0.16em] text-[#2fa59b]">{locale === 'en' ? 'HEALTH FOR BUSINESSES & SELF-EMPLOYED' : 'SALUD PARA EMPRESAS Y AUTÓNOMOS'}</span><strong className="mt-1 block font-heading text-xl text-[var(--blue-deep)]">{locale === 'en' ? 'Health for your team or activity' : 'Salud para tu equipo o actividad'}</strong><span className="mt-1 block text-sm leading-6 text-[var(--muted)]">{locale === 'en' ? 'A shared route with product-specific conditions.' : 'Una ruta común con condiciones específicas de cada producto.'}</span></span></span><ArrowRight className="h-5 w-5 shrink-0 text-[var(--blue)] transition-transform group-hover:translate-x-1" aria-hidden="true" />
          </Link>
