...
export function CookieAwareMap() {
  ...
  if (!canLoadMap) {
    return (
      <div className="flex h-[200px] flex-col items-center justify-center gap-3 bg-[var(--bg-soft)] p-4 text-center text-sm text-[var(--muted)]">
        <p>{isEnglish ? 'The interactive Google Maps map loads only if you accept non-essential cookies.' : 'El mapa interactivo de Google Maps se carga solo si aceptas cookies no necesarias.'}</p>
        <a
        ...
        >
          {isEnglish ? 'Open location in Google Maps' : 'Abrir ubicación en Google Maps'}
        </a>
      </div>
    );
  }
  ...
  return (
    <iframe
      ...
      title={isEnglish ? 'Location in Boadilla del Monte, Madrid' : 'Ubicación en Boadilla del Monte, Madrid'}
    />
  );
}
